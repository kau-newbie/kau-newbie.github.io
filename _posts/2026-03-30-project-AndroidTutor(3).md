...
``` 
자세한 설명을 보니 런타임에서 AGP가 9이상이어야 된댄다.
![버전을 이리저리 바꿔보았지만...](../assets/images/forPost/AndroidTutor(2)/builderrorExample1.png)
대충 요 사진에서 두 번째 줄이 AGP 9.0 이상이어야 된다는 내용이었다. 위 사진은 AGP 바꾸기 싫어서(libs.versions.toml 파일에서 확인해보니, 8.13.2였다.) 2.3.6 이 찾아보니 없다는 내용이었다. (https://github.com/google/ksp/releases 에서 찾아보는 거 아니었나..? 어렵다.)
```
...
```
이라는데, 왜 안됐니....
빌드 오류의 링크 중 하나를 클릭해서, AGP 9.3.1을 다운받았다. 어이쿠, 문제가 많다. 이제 더이상 kotlin을 외부에서 가져오는 게 아니라, 내부 코틀린을 써야한댄다. 호다닥 build.gradle.kts (모듈(app)수준부터 프로젝트 수준까지) kotlin 어쩌구 들을 지웠다. 하지만, 여전히 빌드는 안됐다. ~~c로 돌아가고 싶다.~~ 해당 migration에 관한 공식문서를 찾아봤다.
```
...
아, 나는 옛날 것을 쓰고 있었구나... 또 한세월 안드로이드 스튜디오를 업그레이드 했다. 나는 일각고래 좋아하는데..., 강제로 판다가 됐다.
