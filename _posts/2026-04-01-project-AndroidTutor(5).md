...
3번은 너무 느리다. 어느세월에 다시 LLM에게 쿼리를 보내고, 추론시간을 기다려 완성된 응답을 받아오지? 이미 accessibility_service_config.xml파일에서 android:notificationTimeout="300" 해당 flag 값을 0.3초로 설정해뒀다. 물론, 이것보다는 collect에서 .debounce(3000) 메소드로 3초간 stateFlow에서 갱신되는 상태 값들의 반영을 지연시키는게 더 크다.
...
1. stateFlow로 event 발생 view의 resouce ID, class를 저장한다.
2. 그 다음 이벤트에 대해 1번에서 저장한 정보들과 비교한다.
3. view가 똑같으면 2번은 통과
4. 3번이나 같은 id, class이고, visibility나 focusability가 바뀐게 아니라면, return.
- 단, 사용자의 조작은 무조건 통과. (한 1초에 5번 제한둘까?)
